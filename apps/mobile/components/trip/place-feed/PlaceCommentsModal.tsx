...
import {
  addTripPlaceComment,
  getTripPlaceComments,
  removeTripPlaceComment,
  type TripPlaceComment,
} from '../../../lib/apis/trips';
import colors from '../../../lib/constants/colors';

interface PlaceCommentsModalProps {
  visible: boolean;
  tripId: string;
  placeId: string;
  placeName: string;
...
function PlaceCommentsModal({ visible, tripId, placeId, placeName, onClose }: PlaceCommentsModalProps) {
  ...
  const { data: comments = [], isPending } = useQuery({
    queryKey: ['trip', tripId, 'places', placeId, 'comments'],
    queryFn: () => getTripPlaceComments(tripId, placeId),
    enabled: visible && tripId.length > 0 && placeId.length > 0,
  });

  const invalidateComments = async () => {
    await Promise.all([
      queryClient.invalidateQueries({ queryKey: ['trip', tripId, 'places', placeId, 'comments'] }),
  ...
  const { mutate: deleteComment, isPending: isDeleting } = useMutation({
    mutationFn: (commentId: string) => removeTripPlaceComment(tripId, placeId, commentId),
    onSuccess: async () => {
      await invalidateComments();
    },
  });
  ...
  const handleDelete = (comment: TripPlaceComment) => {
...
export default PlaceCommentsModal;
